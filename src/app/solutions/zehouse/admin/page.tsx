...
export default function ZehouseAdmin() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [advRes, campRes, appRes] = await Promise.all([
      ...
      if (advRes.data) setAdvertisersList(advRes.data);
      if (campRes.data) setCampaignsList(campRes.data);
      if (appRes.data) setApplicationsList(appRes.data);
    } catch (e) {
      console.error('Error loading advertiser data:', e);
    }
    try {
